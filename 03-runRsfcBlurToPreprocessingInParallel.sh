...
for subject in $subjects ; do
    ...
    outputScriptName=run/run-rsfcPreproc-${subject}.blurto.sh
...
[[ -d rsfcPreprocessed.blurto ]] && rm -fr rsfcPreprocessed.blurto
script=${subject}.rsfc.preproc.blurto.tcsh
...
$scriptDir/afni_restproc.py \\
	...
	-epi  $rsfcPrefix \\
	-script \${script} \\
	-dest $DATA/$subject/rsfcPreprocessed.blurto \\
	-prefix $subject.pm \\
	-despike on \\
	-anat_has_skull no \\
	-tsnr \\
...
(
cd $DATA/$subject/rsfcPreprocessed.blurto/tmp
...
EOF
    ...
    rm -f ../log/$subject-rsfc-preproc.blurto.log
    ##qsub -N rsfc-$subject -q all.q -j y -m n -V -wd $( pwd )  -o ../log/$subject-rsfc-preproc.blurto.log $outputScriptName
