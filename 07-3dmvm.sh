...
taskFile="$scriptsDir/3dmvm-taskFile.txt"
...
for seed in $seeds ; do
    ...
    if [[ 1 == 1 ]] ; then 
    cmd="3dMVM      -prefix $GROUP_RESULTS/restingstate.mddAndCtrl.group.and.gender.$seedName.mvm.bucket.$timestamp \
	  -jobs     8 \
	  -mask     $mask \
	  -bsVars   'Group*Gender+Full' \
          -qVars    'Full' \
	  -SS_type  3 \
          -num_glt  4 \
