...
for seed in $seeds ; do
    ...
    echo "*** Performing 2-sample (ctrl vs. mdd) t-test for the $seedName seed"
	...
	3dttest++ -mask mask.grey.$groups.union.masked+tlrc.HEAD \
		  -setA $GROUP_DATA/restingstate.bucket.mddOnly.${seedName}.masked+tlrc.HEAD  \
		  -setB $GROUP_DATA/restingstate.bucket.ctrlOnly.${seedName}.masked+tlrc.HEAD \
    ...
    # 	echo "*** Using mask.grey.$groups.union.masked+tlrc.HEAD as mask"
    # 	3dttest++ -mask mask.grey.$groups.union.masked+tlrc.HEAD \
    # 		  -setA $GROUP_DATA/restingstate.bucket.ctrlOnly.${seedName}.masked+tlrc.HEAD \
    # 		  -setB $GROUP_DATA/restingstate.bucket.mddOnly.${seedName}.masked+tlrc.HEAD  \
    # 		  -BminusA \
    # 		  -prefix ttest.${seedName}.BminusA
    # else 
    # 	3dttest++ -mask $MDD_STANDARD/MNI152_T1_3mm_brain_mask.nii.gz \
    # 		  -setA $GROUP_DATA/restingstate.bucket.ctrlOnly.${seedName}.masked+tlrc.HEAD \
    # 		  -setB $GROUP_DATA/restingstate.bucket.mddOnly.${seedName}.masked+tlrc.HEAD  \
    ...
    # if [[ -f $GROUP_DATA/covariates.mddAndCtrl.${seedName}.tab ]] ; then
    # 	if [[ -f mask.grey.$groups.union.masked+tlrc.HEAD ]] ; then
    # 	    echo "*** Using mask.grey.$groups.union.masked+tlrc.HEAD as mask"
    # 	    3dttest++ -mask mask.grey.$groups.union.masked+tlrc.HEAD \
    # 		      -setA $GROUP_DATA/restingstate.bucket.ctrlOnly.${seedName}.masked+tlrc.HEAD \
    # 		      -setB $GROUP_DATA/restingstate.bucket.mddOnly.${seedName}.masked+tlrc.HEAD  \
    # 		      -BminusA \
    # 		      -covariates $GROUP_DATA/covariates.mddAndCtrl.${seedName}.tab \
    # 		      -prefix ttest.${seedName}.covariates.BminusA
    # 	else 
    # 	    3dttest++ -mask $MDD_STANDARD/MNI152_T1_3mm_brain_mask.nii.gz \
    # 		      -setA $GROUP_DATA/restingstate.bucket.ctrlOnly.${seedName}.masked+tlrc.HEAD \
    # 		      -setB $GROUP_DATA/restingstate.bucket.mddOnly.${seedName}.masked+tlrc.HEAD  \
