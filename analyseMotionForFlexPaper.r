...
for ( task.dir in c("rsfcPreprocessed", "estopPreprocessed") ) {
    ...
    for (subject in allSubjects$subject) {
        ...
        censorFile=file.path(data.dir, subject, task.dir, paste(subject, "pm.censor.1D", sep="."))
        cleanedEPI=file.path(data.dir, subject, task.dir, sprintf("%s.pm.cleanEPI+orig.HEAD", subject))
    ...
    for ( func in c("min", "mean", "max")) { 
        ...
        for (subject in censor.complete.df$subject) {
            ...
            motionFile=file.path(data.dir, subject, task.dir, "tmp", sprintf("%s%s.zp.float.despike_tsh_vr_motion.tcat.1D", subject, task))
