...
graphRegressions <- function(melted.mgd, group.results.dir, rvVariable, rvName, seedName, indicate.treatments=FALSE, in.run.correlations=TRUE) {
    ...
    for ( level in levels(melted.mgd$cluster) ) {
        ...
        if (grepl("scaled", group.results.dir, fixed=TRUE) && is.expression(rvName) ) {
        ...
        if ( indicate.treatments ) {
            ...
            graph = graph + geom_point(mapping=aes_string(color="Treatment", shape="Medication"))
            graph = graph + scale_color_manual(name="Treatment:",
