...
function makeBucketFilePrefix {
    ...
    if [[ $booted -eq 1 ]] ; then 
	prefix="sgacc.masked.restingstate.reversed.formula.${group}.${seedName}.${rvName}.booted.rlm.bucket"
    else
	prefix="sgacc.masked.restingstate.reversed.formula.${group}.${seedName}.${rvName}.rlm.bucket"
    fi
...
if [ $useInherentSmoothness -eq 1 ] ; then 
	...
	usedFwhmX=$( cat $GROUP_DATA/${task}.mddAndCtrl.REML.fwhmEstimates.tab | gawk '{print $1}' | gawk '{s+=$1}END{print s/NR}' )
	usedFwhmY=$( cat $GROUP_DATA/${task}.mddAndCtrl.REML.fwhmEstimates.tab | gawk '{print $2}' | gawk '{s+=$1}END{print s/NR}' )
	usedFwhmZ=$( cat $GROUP_DATA/${task}.mddAndCtrl.REML.fwhmEstimates.tab | gawk '{print $3}' | gawk '{s+=$1}END{print s/NR}' )
	## A = average
	usedFwhmA=$( cat $GROUP_DATA/${task}.mddAndCtrl.REML.fwhmEstimates.tab | gawk '{print $4}' | gawk '{s+=$1}END{print s/NR}' )	
    else 
	usedFwhm=4.2
    fi
else 
...
for seed in $seeds ; do
	...
	if [[ -f $SEEDS/sgACC.mask.3mm+tlrc.HEAD ]] ; then
	    echo "*** Using mask for clustsim: $SEEDS/sgACC.mask.3mm+tlrc.HEAD"
	    # -OKsmallmask 
	    3dClustSim -mask  $SEEDS/sgACC.mask.3mm+tlrc.HEAD -fwhm ${usedFwhm} -niml -both -prefix ${cstempPrefix} -pthr $pValue -athr $cPvalue
	else
	    echo "*** Don't know what mask to use. Stopping."
	    exit 1
