...
demographicsFilename=file.path(admin.data.dir, "0-data_entry_current_2014_afsp.csv")
...
for (seed in seeds) {
    ...
    mgd=cbind(subjectOrder, bucketList, demographics[match(subjectOrder$subject, demographics$ID), c("Grp", "Group", "Gender", "DOB", "MRI")])
    mgd[mgd$subject=="378", "Gender"]="F"
    mgd$subject=paste(mgd$subject, "A", sep="_")
    colnames(mgd)=c("Subj", "InputFile", "Dx", "Group", "Gender", "DOB", "MRI")
    
    mgd=fixDates(mgd)
    mgd=computeAge(mgd)
    ## reorder the columns to suit 3dMVM
    mgd=mgd[, c("Subj", "Dx", "Group", "Gender", "age.in.years", "InputFile")]
