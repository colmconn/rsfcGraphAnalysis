...
analyse <- function(inData, group.variable="Grp", compute.effect.size=TRUE, results.table.filename=NULL) {
    ...
    csvLine=paste("Number of participants (n)", gender.table["Sum", group1], gender.table["Sum", group2], ",,,,,", sep=',')
    ...
    for (i in 1: length(psychMeasures)) {
        ...
        if ( ! variable %in% colnames(inData)) {
            ...
            if (doNonParametricTestsOnVariable(variable)) {
                ...
                group1.string=makeTableString(sm.df[1, 1], sm.df[1, variable],  sm.df[1, "se"], sm.df[1, "min"], sm.df[1, "max"], sm.df[1, "nacount"], inMissingData=TRUE)
                group2.string=makeTableString(sm.df[2, 1], sm.df[2, variable],  sm.df[2, "se"], sm.df[2, "min"], sm.df[2, "max"], sm.df[2, "nacount"], inMissingData=TRUE)
            }
            ...
            if (is.list(test)) {
                ...
                if (doNonParametricTestsOnVariable(variable)) {
                    ...
                    if (var.pvalue < 0.1) {
                        if ( compute.effect.size ) {
                            es=tes(var.statistic, length(inData[inData[, group.variable]==group1, variable]), length(inData[inData[, group.variable]==group2, variable]))
                            ## upper bound on the 95% Confidence interval for the effect size
...
original.control.subjectList.filename=file.path(config.data.dir, "ncl.subjectList.txt")
...
if (! vbmAnalysis) {
    ...
    original.n.table=addmargins(original.n.table)
    print(original.n.table)
    ...
    nodata.n.table=addmargins(nodata.n.table)
    print(nodata.n.table)
    print(nodata.n.test)
    ...
    if (dim(doNotAnalyze.subjectList.mgd)[1] > 0) {
        ...
        doNotAnalyze.n.table=addmargins(doNotAnalyze.n.table)
        print(doNotAnalyze.n.table)
...
##     stop("Cannot continue with those MDD subjects included in the analysis\n")
## }
