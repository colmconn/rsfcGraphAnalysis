...
function makeFwhmFiles {
	...
	if [[ ! -f $DATA/$subject/afniRsfcPreprocessed/00_DO_NOT_ANALYSE_${subject}_${excessiveMotionThresholdPercentage}percent.txt ]] ; then
	   
	    ## cat $DATA/$subject/afniRsfcPreprocessed/${subject}.cleanEPI.blur.est.1D >> $remlFwhmFile

	    cat  $DATA/$subject/afniRsfcPreprocessed/acf.blur.errts.1D  | sed "2q;d" >> $remlFwhmFile
	fi
    done
}

function makeAcfFiles {
    local groupName="$1"
    local subjectList="$2"
    local groupData="$3"
    
    remlAcfFile=$groupData/restingstate.$groupName.acfEstimates.tab
    cat /dev/null > $remlAcfFile
    for subject in $subjectList ; do
	if [[ ! -f $DATA/$subject/afniRsfcPreprocessed/00_DO_NOT_ANALYSE_${subject}_${excessiveMotionThresholdPercentage}percent.txt ]] ; then
	   
	    ## cat $DATA/$subject/afniRsfcPreprocessed/${subject}.cleanEPI.blur.est.1D >> $remlAcfFile

	    cat  $DATA/$subject/afniRsfcPreprocessed/acf.blur.errts.1D  | sed "4q;d" >> $remlAcfFile
	fi
    done
}
...
function makeAutocorrelatedBuckets {
	...
	subbrikLabels=""
	for subject in $subjectList; do
	    zScoreFile=$DATA/$subject/afniRsfc/$seedName/${seedName}.z-score+tlrc.HEAD
	    if [[ ! -f $DATA/$subject/afniRsfcPreprocessed/00_DO_NOT_ANALYSE_${subject}_${excessiveMotionThresholdPercentage}percent.txt ]] ; then
		...
		echo "*** WARNING: Found $DATA/$subject/afniRsfcPreprocessed/00_DO_NOT_ANALYSE_${subject}_${excessiveMotionThresholdPercentage}percent.txt. Skipping ${subject}."
	    fi
...
function maskBuckets {
	...
	if [[ -f $GROUP_RESULTS/mask.grey.$groups.union.masked+tlrc.HEAD ]] ; then
	    ...
	    3dcalc -datum float -a $MDD_STANDARD/MNI_caez_N27_brain.3mm+tlrc.HEAD -b ${restingStateBucketFile} -expr "a*b" -prefix ${restingStateBucketFile%%+*}.masked
	fi
	rm -f ${restingStateBucketFile}.*
    done ## end of for seed in $seeds ; do 
...
seedName="CAEZ_L_whole_amygdala.3mm"
## seedName="L_whole_sgacc.3mm"

if [[ $makeBetweenGroupBuckets == 1 ]] ; then 

    GROUP_DATA=$DATA/Group.data.afni #.kaiser.amygdala.paper
    GROUP_RESULTS=$DATA/Group.results.afni #.kaiser.amygdala.paper
...
#    mddSubjects="$( cat ../data/config/mdd_subjectList_npp.txt )"
    ...
    makeAcfFiles "mddAndCtrl" "$subjects"     $GROUP_DATA
    makeAcfFiles "ctrlOnly"   "$ctrlSubjects" $GROUP_DATA
    makeAcfFiles "mddOnly"    "$mddSubjects"  $GROUP_DATA
