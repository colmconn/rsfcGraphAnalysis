...
for seed in $seeds ; do
    ...
    3dcalc -datum float -a ${seed} -b ${preprocessedRsfcDir}/${subjectNumber}.pm.cleanEPI.MNI.nii.gz -expr "a*b" -prefix ${preprocessedRsfcDir}/__tmp__${seedName}__${subjectNumber}.pm.cleanEPI.MNI
    ## don't use 3dROIstast with mask data sets that are made of non-whole number values
    ## 3dROIstats -quiet -mask_f2short -mask ${seed} ${preprocessedRsfcDir}/__tmp__${seedName}__${subjectNumber}.pm.cleanEPI.MNI+tlrc.HEAD  > ${seedName}/${seedName}.ts.1D
    3dmaskave -quiet -mask ${seed} ${preprocessedRsfcDir}/__tmp__${seedName}__${subjectNumber}.pm.cleanEPI.MNI+tlrc.HEAD  > ${seedName}/${seedName}.ts.1D
