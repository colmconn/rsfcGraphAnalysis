...
ctrl.subjectOrder.filename=file.path(config.data.dir, "ncl_subjectList_npp.txt")
mdd.subjectOrder.filename=file.path(config.data.dir,  "mdd_subjectList_npp.txt")
...
mdds=subset (mgd, Grp == "MDD")
ncls=subset (mgd, Grp == "NCL")

cat("*** Quantiles for MDD WASI Full score\n")
lower.quantile=0.10
upper.quantile=0.90
mdd.full.lower.quantile=quantile(mdds$Full, lower.quantile)
mdd.full.upper.quantile=quantile(mdds$Full, upper.quantile)
## ncl.full.lower.quantile=quantile(ncls$Full, lower.quantile)
## ncl.full.upper.quantile=quantile(ncls$Full, upper.quantile)

cat(sprintf("### MDD: Lower %0.0f%% quantile = %0.2f\n", lower.quantile * 100,  mdd.full.lower.quantile))
cat(sprintf("### MDD: Upper %0.0f%% quantile = %0.2f\n", upper.quantile * 100,  mdd.full.upper.quantile))
## ## cat(sprintf("### NCL: Lower %0.2f%% quantile = %0.2f\n", lower.quantile * 100,  ncl.full.lower.quantile))
## ## cat(sprintf("### NCL: Upper %0.2f%% quantile = %0.2f\n", upper.quantile * 100,  ncl.full.upper.quantile))

between.quantiles.mdds=subset(mdds, Full > mdd.full.lower.quantile & Full < mdd.full.upper.quantile)
## between.quantiles.ncls=subset(ncls, Full > ncl.full.lower.quantile & Full < ncl.full.upper.quantile)
mgd=rbind(between.quantiles.mdds, ncls)

## m = matchControls(Grp ~ Gender + age.in.years + TannerAvg + Full, data=mgd, replace=FALSE, contlabel="NCL")
m = matchControls(Grp ~  Full, data=mgd, replace=FALSE, contlabel="NCL")
mgd <- rbind(mgd[m$controls, ], mgd[m$cases, ])
rownames(mgd) = NULL


full.boxplot=ggplot(mgd, aes(x=Grp, y=Full, fill=Grp)) +
    geom_boxplot() +
        labs(x="Group", y="WASI Full Score") +
            scale_fill_brewer(name="Group:", palette="Set1") +
                my.theme
## dev.new(); print(full.boxplot)

ggsave("wasi.full.boxplot.png", full.boxplot)
