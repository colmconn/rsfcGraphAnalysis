...
## regressionVariables="CDRSR.diff MASC.tscore.diff BDI.diff CGAS.diff CDI.diff RADS.Total.Tscore.diff"

## regressionVariables="BDI.diff"
#regressionVariables="CGAS.diff"

regressionVariables="RADS.Total.Tscore"
...
for regressionVariable in $regressionVariables ; do
	...
	if [[ ! -f $latestRlmBucketFile ]] ; then
	    echo "*** No such file: $latestRlmBucketFile"
	    echo "*** Skipping."
	    continue
	fi
	...
	for polarity in positive negative ; do
		...
		3dcalc -a $latestRlmBucketFile\[$coefBrikId\] -b $latestRlmBucketFile\[$tvalueBrikId\] -expr 'a*isnegative(a)' -prefix rm.${outputPrefix}.coefficient.$polarity 
