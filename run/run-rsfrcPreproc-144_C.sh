...
/home/colmconn/bin/afni_restproc.py \
	...
	-epi  /data/sanDiego/rsfcGraphAnalysis/data/144_C/144_Cfuncon+orig \
	-script 144_C.rsfc.preproc.tcsh \
	-dest /data/sanDiego/rsfcGraphAnalysis/data/144_C/rsfcPreprocessed \
	-prefix 144_C.pm \
	-despike on \
	-anat_has_skull no \
	-tsnr \
...
cd rsfcPreprocessed
tcsh -f 144_C.rsfc.preproc.tcsh
...
if [[ -f 144_C.pm.cleanEPI+orig.HEAD ]] ; then 
	...
	if [[ $numberOfCensoredVolumes -gt $cutoff ]] ; then 
	    echo "*** A total of $numberOfCensoredVolumes of $totalNumberOfVolumes we censored which is greater than $threshold % (n=$cutoff) of all total volumes of this subject" > 00_DO_NOT_ANALYSE_144_C_${threshold}percent.txt
