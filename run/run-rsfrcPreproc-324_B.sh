...
/home/colmconn/bin/afni_restproc.py \
	...
	-epi  /data/sanDiego/rsfcGraphAnalysis/data/324_B/324_Bfuncon+orig \
	-script 324_B.rsfc.preproc.tcsh \
	-dest /data/sanDiego/rsfcGraphAnalysis/data/324_B/rsfcPreprocessed \
	-prefix 324_B.pm \
	-despike on \
	-anat_has_skull no \
	-tsnr \
...
cd rsfcPreprocessed
tcsh -f 324_B.rsfc.preproc.tcsh
...
if [[ -f 324_B.pm.cleanEPI+orig.HEAD ]] ; then 
	...
	if [[ $numberOfCensoredVolumes -gt $cutoff ]] ; then 
	    echo "*** A total of $numberOfCensoredVolumes of $totalNumberOfVolumes we censored which is greater than $threshold % (n=$cutoff) of all total volumes of this subject" > 00_DO_NOT_ANALYSE_324_B_${threshold}percent.txt
