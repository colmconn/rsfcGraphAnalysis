...
/home/colmconn/bin/afni_restproc.py \
	...
	-epi  /data/sanDiego/rsfcGraphAnalysis/data/304_C/304_Cfuncon+orig \
	-script 304_C.rsfc.preproc.tcsh \
	-dest /data/sanDiego/rsfcGraphAnalysis/data/304_C/rsfcPreprocessed \
	-prefix 304_C.pm \
	-despike on \
	-anat_has_skull no \
	-tsnr \
...
cd rsfcPreprocessed
tcsh -f 304_C.rsfc.preproc.tcsh
...
if [[ -f 304_C.pm.cleanEPI+orig.HEAD ]] ; then 
	...
	if [[ $numberOfCensoredVolumes -gt $cutoff ]] ; then 
	    echo "*** A total of $numberOfCensoredVolumes of $totalNumberOfVolumes we censored which is greater than $threshold % (n=$cutoff) of all total volumes of this subject" > 00_DO_NOT_ANALYSE_304_C_${threshold}percent.txt
