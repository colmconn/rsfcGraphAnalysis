...
class RestInterface:
   ...
   def init_options(self):
      ...
      self.valid_opts.add_opt('-wmmask', 1, [], req=0, helpstr='White matter mask')
      self.valid_opts.add_opt('-vmask',  1, [], req=0, helpstr='Ventricle mask')            
      ...
      self.valid_opts.add_opt('-cleanOnly', 0, [], req=0, helpstr = 'whether the EPI data should be cleaned only')
      self.valid_opts.add_opt('-zeropadfirst', 1, [], req=0, helpstr = 'whether the EPI data should be zero padded before conversion to a float dataset')            
   ...
   def process_options(self):
      ...
      for opt in uopts.olist:
         if opt.name == '-anat':
            ...
            self.dreg = True
         elif opt.name == '-cleanOnly':
            self.cleanOnly = True
         elif opt.name == "-zeropadfirst":
            val,err = uopts.get_string_opt('', opt=opt)
            if err: return 1
            self.zeropadFirst = True
            self.zeropadArgs = val
            print "INFO: Using \"%s\" as argument to 3dZeropad command" % ( self.zeropadArgs )
      ...
      if not self.align and (self.outcensor or self.dvarscensor):
         ...
         sys.exit(1)
      if self.aseg and (self.wmmask or self.vmask):
         print "ERROR: -aseg cannot be specified with -wmmask and/or -vmask"
         sys.exit(1)
      if not self.wmmask or not self.vmask :
         print "ERROR: -wmmask and -vmask must be specified together"
         sys.exit(1)
   ...
   def using_aseg(self):
      """A function that returns TRUE iff we are using the aseg file
      derived from Freesurfer."""

      if self.aseg and not self.wmmask and not self.vmask:
         return True
      return False

   def using_nonfreesurfer_masks(self):
      """A function that returns TRUE iff we are *not* using the aseg
      file derived from Freesurfer and using GM, WM and ventricle
      masks derived from some other method, e.g. 3dSeg or FSL's
      FAST."""

      if not self.aseg and self.wmmask  and self.vmask:
         return True
      return False
   ...
   def zeropad(self, curEPI):
      ## zeropad the EPI before it is converted to a float dataset
      self.info("Zeropad the EPI dataset before it is converted into a float value dataset")
      epip,epis = remove_suffix(curEPI)
      nextEPI = epip + ".zp"
      cmd = "3dZeropad %s -prefix %s %s" % (self.zeropadArgs, nextEPI, curEPI)
      self.write_execute(cmd)
      return nextEPI + epis
   ...
   def align_etc_nonfreesurfer(self, curEPI, curANAT, cur_WM_MASK, cur_V_MASK, curALIGN, curREGMASKS):
      if not self.align:
         return curEPI,curANAT,None,cur_WM_MASK, cur_V_MASK, None,curREGMASKS 
      if self.tlrc:
         #tlrc, tshift, and register
         return self.tlrc_all_nonfreesurfer(curEPI,curANAT, cur_WM_MASK, cur_V_MASK, curREGMASKS)
      else:
         #or tshift, register, and align
         return self.shift_reg_align_nonfreesurfer(curEPI,curANAT, cur_WM_MASK, cur_V_MASK, curALIGN,curREGMASKS)
   ...
   def tlrc_all_nonfresurfer(self, curEPI, curANAT, cur_WM_MASK, cur_V_MASK, curREGMASKS):
      #tlrc the anatomical, then align epi to anat
      #also tshift and register at the same time, minimizing interpolation
      #return the names of the motion params and the aligned EPI
      #also transform the aseg file to tlrc space
      ...
      ## tlrc the GM and WM masks under the assumption that they were
      ## previously aligned to the original anatomy
      if cur_WM_MASK and cur_V_MASK:
         for msk in [ cur_WM_MASK, cur_V_MASK ] :
            prefix,suffix = remove_suffix(msk)
            prefix_tlrc=prefix + "+tlrc"
            cmd="@auto_tlrc -onewarp -apar %s -rmode NN -input %s " % (anatt, msk)
            self.write_execute(cmd)
      else:
         wmmask_t=None
         vmask_t=None         
      ...
      if self.tshift:
         ...
         motparams = epip + "_vr_motion.1D"    
      return epit,anatt,motparams, wmmask_t, vmask_t, epip + "_al_tlrc_mat.aff12.1D",curREGMASKS
   ...
   def shift_reg_align_nonfreesurfer(self, curEPI, curANAT, cur_WM_MASK, cur_V_MASK, curALIGN, curREGMASKS):
      #tshift and register epi 
      #also align anat to epi
      #also align the aseg file
      epip,epis = remove_suffix(curEPI)
      ...
      if self.epi2anat:
         ...
         #align the GM and WM masks, assuming they started in alignment with the anat
         if cur_WM_MASK and cur_V_MASK:
            prefix,suffix = remove_suffix(cur_WM_MASK )
            cmd = "3dAllineate -final NN -1Dmatrix_apply %s_al_mat.aff12.1D -prefix %s_al %s" % (anatp, \
                                                                                                 prefix, cur_WM_MASK  )
            self.write_execute(cmd)
            wmmask_al = prefix + "_al" + suffix

            prefix,suffix = remove_suffix(cur_V_MASK )
            cmd = "3dAllineate -final NN -1Dmatrix_apply %s_al_mat.aff12.1D -prefix %s_al %s" % (anatp, \
                                                                                                 prefix, cur_V_MASK  )
            self.write_execute(cmd)
            vmask_al = prefix + "_al" + suffix
         else:
            wmmask_al=None
            vmask_al=None                        
      ...
      if self.tshift:
         ...
         mot = epip + "_vr_motion.1D"
      return epi_al, anat_al, mot, wmmask_al, vmask_al, xform, curREGMASKS
   ...
   def erode_wm_mask(self, curEPI, cur_WM_MASK):
      wmprefix,wmsuffix=remove_suffix(cur_WM_MASK)
      cmd = "3dresample -master %s -rmode NN -inset %s -prefix mask.WM.resample" % (curEPI,cur_WM_MASK)
      self.write_execute(cmd)

      cmd = "3dcalc -a mask.WM.resample%s -b a+i -c a-i -d a+j -e a-j -f a+k -g a-k -expr \'a*(step(step(b) + step(c) + step(d) + step(e) + step(f) + step(g) - %d))\' -prefix mask.WM.resample.erode" % (wmsuffix, (6 - int(self.wmerode)))
      self.write_execute(cmd)
      if self.globalwm:
         return "mask.WM.resample.erode%s" % wmsuffix
      else:
         return ["mask.WM.resample.erode%s" % wmsuffix, self.wmsize]
   ...
   def erode_v_mask(self, curEPI, cur_V_MASK):
      vprefix,vsuffix=remove_suffix(cur_V_MASK)
      cmd = "3dresample -master %s -rmode NN -inset %s -prefix mask.vent.resample" % (curEPI,cur_V_MASK)
      self.write_execute(cmd)

      cmd = "3dcalc -a mask.vent.resample%s -b a+i -c a-i -d a+j -e a-j -f a+k -g a-k -expr \'a*(step(step(b) + step(c) + step(d) + step(e) + step(f) + step(g) - %d))\' -prefix mask.vent.resample.erode" % (vsuffix, (6 - int(self.venterode)))
      self.write_execute(cmd)
      return "mask.vent.resample.erode%s" % vsuffix
   ...
   def process_data(self):
      ...
      elif self.using_nonfreesurfer_masks():
         cur_WM_MASK=copy_input(self.wmmask, origdir)
         cur_V_MASK=copy_input(self.vmask, origdir)
      ...
      ## zeropad the EPI before conversion to float only if requested
      ## by the user
      if self.zeropadFirst:
         curEPI = self.zeropad(curEPI)
      ...
      if self.using_nonfreesurfer_masks():
         curEPI,curANAT,curMOTION, cur_WM_MASK, cur_V_MASK, epiXFORM,curREGMASKS = self.align_etc_nonfreesurfer(curEPI, curANAT, cur_WM_MASK, cur_V_MASK, curALIGN, self.regmasks)
      else:
      ...
      if self.using_nonfreesurfer_masks():
         maskWM = self.erode_wm_mask(curEPI, cur_WM_MASK)
      else:
      ...
      if self.using_nonfreesurfer_masks():
         maskVENT = self.erode_v_mask(curEPI, cur_V_MASK)
      else:
          #prep ventricle mask
...
def remove_path_and_convert(dset):
   ...
   if infile[-4:] == ".mgz":
      ...
      #self.write_execute(cmd)
      infile = inp + ".nii"
   ## handle any type of compressed or uncompressed NIfTI file
   niftiMatch=re.search(r"(.*)(\.nii.*)$", infile)   
   if niftiMatch:
      inp = niftiMatch.group(1)
